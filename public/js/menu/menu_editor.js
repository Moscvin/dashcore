...
function MenuEditor(idSelector, options) {
    ...
    $(document).on('click', '.btnRemove', function (e) {
        ...
        if (confirm("Esta seguro que desea eliminar el item?")){
            ...
            if($(this).parent().parent().parent().find(' > ul').attr("style")){
                console.log(222);
            }
    ...
    this.update = function(){
        ...
        $cEl.find('span.txt').first().text($cEl.data('description'));
        resetForm();
    };
    ...
    this.add = function(){
        ...
        let textItem = $('<span>').addClass('txt').text(data.description);
    ...
    function createMenu(arrayItem, depth) {
        ...
        $.each(arrayItem, function (k, v) {
            ...
            var $li = $('<li>');
            var descc = v.description;
            $li.attr('id', descc.replace(' ', '') );
            ...
            var $span = $('<span>').addClass('txt').append(v.description);
